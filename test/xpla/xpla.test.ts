...
const options: BlockchainClientOptions = {
  ...
  // nodeURL: 'http://34.146.148.127:1317',
  chainID: 'cube_47-5',
  // isClassic: true
}
...
describe('Connection', () => {
  describe('wallet test', function () {
    let wallet: any
    before(
      async () => (wallet = xpla.client.wallet(test1)),
    )
    ...
    it('get Balance', async function () {
      ...
      const ttResult = await tt.bank.balance('xpla16v6y48xllwy7amcmvhkv0a3zp7jepl44yvhvxt')
      const result = await xpla.client.getBalance('xpla16v6y48xllwy7amcmvhkv0a3zp7jepl44yvhvxt')
      expect(result).not.to.be.null
  ...
  describe('TX Test', function () {
    ...
    it('get fee', async function () {
      const wallet = xpla.client.wallet(test1)
      const result = await xpla.client.getFee(txObject, wallet)
      expect(result).not.to.be.null
    ...
    it('create tx', async function () {
      const wallet = xpla.client.wallet(test1)
      const fee = await xpla.client.getFee(txObject, wallet)

      const result = await xpla.client.createTx(
        {
          msgs: [transferC2x],
          memo: '',
          fee,
    ...
    it('sign tx', async function () {
      const senderWallet = xpla.client.wallet(test1)
      const fee = await xpla.client.getFee(txObject, senderWallet)
      const unsignedTx = await xpla.client.createTx(
        {
          msgs: [transferC2x],
          memo: '441',
          fee: fee,
      ...
      const result = await xpla.client.signTx(unsignedTx, senderWallet);
      expect(result).not.to.be.null
    ...
    it('send sign tx', async function () {
      ...
      const signedTx = await xpla.client.signTx(unsignedTx, senderWallet);
    ...
    it('contract query', async function () {
      const result = await xpla.client.contractQuery(tokenAddress, {
    ...
    it('get txhash', async function () {
      ...
      const result = await xpla.client.getTx(txhash);
      expect(result).not.to.be.null
